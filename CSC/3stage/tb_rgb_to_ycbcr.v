...
module tb_rgb_to_ycbcr();
    ...
    // 定点系数: Y  = 47R + 157G + 16B + 4096
    //           Cb = 112B + 32768 - 26R - 86G
    //           Cr = 112R + 32768 - 102G - 10B
    ...
    always @(posedge clk) begin
        if (!ngreset) begin
            ...
            // 数据校验: DUT输出 vs 定点期望
            if (Y !== csc_round(47*r_ref + 157*g_ref + 16*b_ref + 4096) ||
                Cb !== csc_round(112*b_ref + 32768 - 26*r_ref - 86*g_ref) ||
                Cr !== csc_round(112*r_ref + 32768 - 102*g_ref - 10*b_ref)) begin
                err_cnt <= err_cnt + 1;
                if (err_cnt < 20)
                    $display("FAIL: R=%0d G=%0d B=%0d | got Y=%0d Cb=%0d Cr=%0d | exp Y=%0d Cb=%0d Cr=%0d",
                             r_ref, g_ref, b_ref, Y, Cb, Cr,
                             csc_round(47*r_ref + 157*g_ref + 16*b_ref + 4096),
                             csc_round(112*b_ref + 32768 - 26*r_ref - 86*g_ref),
                             csc_round(112*r_ref + 32768 - 102*g_ref - 10*b_ref));
            end
        end
    end
