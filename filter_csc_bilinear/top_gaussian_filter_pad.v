//========================================================================
// top_gaussian_filter_pad.v —— 高斯滤波顶层（pad 版行缓存，链路 crop 收缩修复用）
// 与 top_gaussian_filter.v 的唯一区别：行缓存换成 line_buffer_3x3_pad.v，
// 输出全尺寸 IMG_H×IMG_W（边缘 replicate 复制填充），不再 crop 少一圈。
// 输入要求：必须带 blanking（h-blank≥1 拍/行末、v-blank≥IMG_W+8 拍/帧末）
// 数据流与核复用：窗口 w11..w33 → 3×gaussian_3x3_8b（R/G/B），同 crop 版
// (本副本的 include 已移除：底层文件由 top_chain.v 统一声明)
//========================================================================
`timescale 1ns/1ps

module top_gaussian_filter_pad #(
    ...
    //========================================================================
    // 3×gaussian_3x3_8b：R/G/B 各一核（对称求和与位置无关，映射同 crop 版）
    //========================================================================
