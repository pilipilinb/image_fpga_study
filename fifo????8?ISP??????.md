...
>
> **验收结果**：四套 TB 全 PASS——双线性/MHC × 协议(16×12×5 帧四场景)/图像(112×103 真图+60 坏点)，期望比对 0 误差 + 出侧稳定性断言零违例 + 收发计数一致（960/960、11536/11536）。输出位宽按用户纠正定为 **RGB888（out_data[23:0]，对齐 VDMA 契约）**，RAW10→8bit 在 Demosaic 出口 >>2 折算。PSNR：Bayer 域(10bit) 28.33→38.84dB（+10.51）；RGB888 域双线性 30.28→40.37dB（+10.09）；MHC 32.60dB（坏点链上高通细节项放大 DPC 残留，低于双线性）。对比图 `dpc_demosaic_compare.png`。
>
