...
> 换工作向 FPGA 图像处理学习项目（2026-08）· 九个子系统已手写实现并仿真验证（W4 收尾中：手写 vs IP 资源对比待办；Month-2 行缓存 + 8 级 ISP 建链已完成 M0 / M0.5 / M1）
...
|---|---|---|---|---|
| BLC 黑电平校正 | Bayer | RAW → RAW | 无（逐像素减偏置，每通道一个偏置） | 未实现（最简单，适合当起点） |
